...
class Mod {
    ...
    preAkiLoad(container) {
        ...
        const LauncherController = container.resolve("LauncherController");
        const profileHelper = container.resolve("ProfileHelper");
        const configServer = container.resolve("ConfigServer");
        const ragfairController = container.resolve("RagfairController");
        Mod.container = container;
        staticRouterModService.registerStaticRouter("StaticRoutePeekingAkiHudLoad", [
            {
        ...
        container.afterResolution("RagfairController", (_t, result) => {
            ...
            result.getItemMinAvgMaxFleaPriceValues = (getPriceRequest) => {
    ...
    postDBLoad(container) {
        ...
        const h1 = crypto_1.default.createHash('sha256');
    ...
    MyMethodForGame(container, sessionId) {
        const logger = container.resolve("WinstonLogger");
        const PreAkiModLoader = container.resolve("PreAkiModLoader");
        const FuncDatabaseServer = container.resolve("DatabaseServer");
        const FuncImporterUtil = container.resolve("ImporterUtil");
    ...
    getItemMinAvgMaxFleaPriceValues(getPriceRequest) {
        // Get all items of tpl (sort by price)
        const ragfairController = Mod.container.resolve("RagfairController");
        const FuncDatabaseServer = Mod.container.resolve("DatabaseServer");
        const ragfairSortHelper = Mod.container.resolve("RagfairSortHelper");
        ...
        if (typeof (offers) === "object" && offers.length > 0) {
            offers = ragfairController.ragfairSortHelper.sortOffers(offers, RagfairSort_1.RagfairSort.PRICE);
